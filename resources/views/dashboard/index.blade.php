 ...
 <ul class="navbar-nav sidebar accordion" id="accordionSidebar">
    ...
    @if(auth()->user()->hasRole('admin'))
        ...
        <li class="nav-item {{ Request::is('mobil*') ? 'active' : '' }}">
            ...
            <div id="collapseCars" class="collapse {{ Request::is('mobil*') ? 'show' : '' }}">
    ...
    @if(auth()->user()->hasRole('teknisi'))
        ...
        <li class="nav-item {{ Request::is('transaksi*') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('teknisi.verifikasi') }}">
        ...
        <li class="nav-item {{ Request::is('riwayat-servis*') ? 'active' : '' }}">
            <a class="nav-link" href="/riwayat-servis">
                <i class="fas fa-fw fa-history"></i> <span>Riwayat Kerja</span>
            </a>
        </li>

        <li class="nav-item {{ Request::is('lapor-kerusakan*') ? 'active' : '' }}">
    ...
    @if(auth()->user()->hasRole('peminjam'))
        ...
        <li class="nav-item {{ Request::is('my-bookings*') ? 'active' : '' }}">
        ...
        <li class="nav-item {{ Request::is('peminjam*') ? 'active' : '' }}">
            <a class="nav-link" href="{{ route('peminjam.booking') }}">
                <i class="fas fa-fw fa-undo-alt"></i> <span>Pengembalian</span>
            </a>
        </li>

       <li class="nav-item {{ Request::is('redeem*') ? 'active' : '' }}">
    <a class="nav-link" href="{{ route('booking.redeem') }}">
    ...
    @endswitch
                ...
                <div class="position-relative">
                    @if(auth()->user()->avatar)
                        <img class="img-profile rounded-circle border border-info shadow-sm" src="{{ asset('storage/' . auth()->user()->avatar) }}" style="width: 40px; height: 40px; object-fit: cover;">
                    @else
                        <img class="img-profile rounded-circle border border-info shadow-sm" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=a2d2ff&color=fff" style="width: 40px; height: 40px;">
                    @endif
...
<div class="modal fade" id="logoutModal" tabindex="-1" role="dialog" aria-labelledby="logoutLabel" aria-hidden="true">
        ...
        <div class="row align-items-center position-relative" style="z-index: 2;">
            <div class="col-lg-7 mb-4 mb-lg-0 text-white">
                ...
                <h1 class="font-weight-bold mb-2" style="font-size: 3rem; letter-spacing: -2px; line-height: 1;">
                    ...
                    @else
                        Hello, <span style="color: #ffe5b4; text-shadow: 0 0 15px rgba(255,229,180,0.4);">{{ explode(' ', auth()->user()->name)[0] }}</span>! ❄️
                    @endif
                </h1>
                ...
                @if(auth()->user()->role_id == 1)
                    <a href="{{ url('/laporan') }}" class="btn btn-white bg-white px-5 py-3 font-weight-bold rounded-pill shadow-lg pulse-button" style="color: #4a90e2; border: none;">
                        Lihat Laporan <i class="fas fa-chart-line ml-2"></i>
                    </a>
                @elseif(auth()->user()->role_id == 2)
                    <a href="{{ url('/maintenance') }}" class="btn btn-white bg-white px-5 py-3 font-weight-bold rounded-pill shadow-lg pulse-button" style="color: #4a90e2; border: none;">
                        Cek Unit <i class="fas fa-wrench ml-2"></i>
                    </a>
                @else
                    <a href="{{ url('/katalog') }}" class="btn btn-white bg-white px-5 py-3 font-weight-bold rounded-pill shadow-lg pulse-button" style="color: #4a90e2; border: none;">
            ...
            <div class="col-lg-5">
                <div class="row no-gutters">
                    @if(auth()->user()->role_id == 1) {{-- Admin View --}}
                        <div class="col-6 p-2">
                            <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm">
                                ...
                                <div class="h3 font-weight-bold text-dark mb-0 mt-1">{{ $stats['total_user'] ?? '0' }}</div>
                            </div>
                        </div>
                       <div class="col-6 p-2">
    <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm" style="border-radius: 20px; background: rgba(255,255,255,0.8); backdrop-filter: blur(10px);">
        ...
        <div class="text-xs text-success font-weight-bold mt-1">Hari Ini</div>
                    ...
                    @elseif(auth()->user()->role_id == 2) {{-- Technician View --}}
                        <div class="col-6 p-2">
                            <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm">
                                ...
                                <div class="h3 font-weight-bold text-danger mb-0 mt-1">{{ $stats['butuh_maintenance'] ?? '0' }}</div>
                            </div>
                        </div>
                        <div class="col-6 p-2">
                            <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm">
                                ...
                                <div class="h3 font-weight-bold text-success mb-0 mt-1">{{ $stats['mobil_tersedia'] ?? '0' }}</div>
                            </div>
                        </div>
                    @else {{-- Peminjam View --}}
                        <div class="col-6 p-2">
                            <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm">
                                ...
                                <div class="h3 font-weight-bold text-dark mb-0 mt-1">{{ $stats['total_sewa_anda'] }}</div>
                            </div>
                        </div>
                        <div class="col-6 p-2">
                            <div class="card border-0 p-4 text-center h-100 justify-content-center shadow-sm">
...
<div class="row mb-4">
   ...
   <div class="d-flex justify-content-between align-items-center mb-3">
    ...
    <div class="progress mb-2" style="height: 8px; border-radius: 10px; background: #f1f5f9;">
        <div class="progress-bar {{ $displayPercentage >= 100 ? 'bg-success' : 'bg-info' }} progress-bar-striped progress-bar-animated" 
             role="progressbar" 
             style="width: {{ $barPercentage }}%; border-radius: 10px;">
        </div>
    </div>

    <div class="d-flex justify-content-between align-items-center small mt-3">
        <div class="text-muted font-weight-bold">
            Target: <span class="text-dark">Rp {{ number_format($target/1000000, 1, ',', '.') }} Jt</span>
        </div>
        <div class="{{ $displayPercentage >= 100 ? 'text-success' : 'text-info' }} font-weight-bold badge badge-light rounded-pill px-2 border">
            {{ number_format($displayPercentage, 1) }}% Achieved
        </div>
    </div>
</div>
        </div>
            ...
            <div class="card-body p-4">
                ...
                <div class="row text-center mb-4 no-gutters bg-light rounded-lg py-3">
                    <div class="col-6 border-right">
                        <div class="h3 font-weight-bold text-dark mb-0">{{ $stats['transaksi_pending'] ?? '3' }}</div>
                        <small class="text-muted font-weight-bold text-uppercase" style="font-size: 0.65rem;">Sewa Baru</small>
                    </div>
                    <div class="col-6">
                        <div class="h3 font-weight-bold text-danger mb-0">{{ $stats['kyc_pending'] ?? '18' }}</div>
                ...
                <div class="px-4 pb-4" style="max-height: 420px; overflow-y: auto; scrollbar-width: none;">
                    ...
                    <div class="p-3 mb-3 transition-up border" 
                        ...
                        <div class="d-flex justify-content-between mb-2">
                            ...
                            <small class="text-muted">{{ $lp->created_at->diffForHumans() }}</small>
                        </div>
                        ...
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="d-flex align-items-center">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($lp->user->name ?? 'S') }}&background=0ea5e9&color=fff" 
                                     class="rounded-circle mr-2" style="width: 24px; height: 24px;">
                                <small class="text-secondary font-weight-bold">{{ $lp->user->name ?? 'Staff' }}</small>
                            </div>
                            @if($lp->status == 'pending')
                                <button onclick="openReplyModal({{ $lp->id }}, '{{ $lp->pesan }}')" 
        ...
        <div class="card border-0 shadow-sm h-100 transition-hover" style="border-radius: 20px; background: #fff; border-left: 5px solid #e53e3e !important;">
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-end justify-content-between">
                    <h2 class="font-weight-bold text-dark mb-0">{{ \App\Models\Mobil::where('status', 'rusak')->count() }} <small class="h6 text-muted">Unit</small></h2>
        ...
        <div class="card border-0 shadow-sm h-100" style="border-radius: 20px; background: #2d3748;">
            <div class="card-body p-3 d-flex align-items-center">
                <div class="mr-3 position-relative">
                    <img src="https://ui-avatars.com/api/?name={{ auth()->user()->name }}&background=4a5568&color=fff" class="rounded-circle border border-light" width="50">
                ...
                <div>
                    ...
                    <div class="text-white font-weight-bold mb-0">{{ explode(' ', auth()->user()->name)[0] }}!</div>
        ...
        <div class="card border-0 shadow-sm mb-4" style="border-radius: 20px;">
            <div class="card-header bg-white border-0 py-4 d-flex align-items-center justify-content-between" style="border-radius: 20px 20px 0 0;">
                ...
                <div class="badge badge-light p-2 rounded-pill px-3">
                    <i class="fas fa-calendar-day text-info mr-2"></i> {{ date('d M Y') }}
                </div>
            </div>
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @php $tugasPerbaikan = \App\Models\Mobil::whereIn('status', ['rusak', 'servis'])->get(); @endphp
                            @forelse($tugasPerbaikan as $tugas)
                            <tr>
                                <td class="pl-4 py-3">
                                    <div class="d-flex align-items-center">
                                        <div class="mr-3 p-2 bg-light rounded-lg text-primary shadow-sm"><i class="fas fa-car-side"></i></div>
                                        <div>
                                            <div class="font-weight-bold text-dark">{{ $tugas->merek }}</div>
                                            <span class="text-muted small font-weight-bold">{{ $tugas->no_polisi }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    @if($tugas->status == 'rusak')
                                ...
                                <td class="text-right pr-4">
                                    <a href="/mobil/{{ $tugas->id }}/edit" class="btn btn-outline-dark btn-sm rounded-pill px-3 font-weight-bold transition-hover">
    ...
    <div class="col-xl-4 col-md-6 mb-4">
        <div class="card border-0 shadow-sm h-100 overflow-hidden" style="border-radius: 20px; background: #fff;">
            ...
            <div class="card-body position-relative">
                ...
                @if($sewaAktif)
                    <h5 class="font-weight-bold text-dark mb-1">{{ $sewaAktif->mobil->merek }} {{ $sewaAktif->mobil->model }}</h5>
                    <div class="badge badge-dark mb-3 px-2 py-1">{{ $sewaAktif->mobil->no_polisi }}</div>
                ...
                @else
                    <div class="text-center py-3">
                        ...
                        <a href="{{ url('/katalog') }}" class="btn btn-primary btn-sm rounded-pill px-4 shadow-sm pulse-button">Cek Katalog</a>
                    </div>
                @endif
            </div>
        </div>
    ...
    @if($latestPromo)
        <div class="promo-card d-flex align-items-center p-3 shadow-sm" style="background: #ffffff; border-radius: 20px; border: 1px solid #e2e8f0; transition: all 0.3s ease;">
            ...
            <div class="flex-grow-1">
                ...
                <div class="d-flex align-items-center">
                    <span class="font-weight-bold text-dark mr-2" id="promoCode" style="font-size: 1rem; letter-spacing: 0.5px;">{{ $latestPromo->code }}</span>
                    <span class="badge badge-soft-danger px-2" style="background: #fef2f2; color: #ef4444; border-radius: 6px; font-size: 0.7rem;">-{{ $latestPromo->discount_percent }}%</span>
                </div>
            </div>

            <button class="btn btn-dark rounded-circle d-flex align-items-center justify-content-center shadow-sm" 
                    onclick="copyCode('{{ $latestPromo->code }}')" 
     ...
     onerror="this.src='https://placehold.co/600x400?text=Gambar+Tidak+Ditemukan'">
                            ...
                            <div class="position-absolute d-flex flex-column gap-2" style="top: 15px; left: 15px;">
                                <span class="badge bg-white text-dark shadow-sm rounded-pill px-3 py-2 small font-weight-bold">
                                    {{ $mobil->category->nama_kategori }}
                                </span>
                                @if($hasDiscount)
                                    <span class="badge bg-danger text-white shadow-sm rounded-pill px-3 py-2 small font-weight-bold">
                                        <i class="fas fa-fire mr-1"></i> Hemat {{ $mobil->diskon }}%
                                    </span>
                                @endif
                            </div>
                        </div>
                        <div class="card-body p-4">
                            <h6 class="font-weight-bold text-dark mb-3">{{ $mobil->merek }} {{ $mobil->model }}</h6>
                            <div class="d-flex gap-2 mb-4">
                                <span class="badge badge-light text-muted border-0"><i class="fas fa-cog text-info mr-1"></i> {{ $mobil->transmisi }}</span>
                                <span class="badge badge-light text-muted border-0"><i class="fas fa-users text-info mr-1"></i> {{ $mobil->kursi }} Seats</span>
                            </div>
                            <div class="d-flex justify-content-between align-items-center pt-3 border-top">
                                <div>
                                    @if($hasDiscount)
                                        <del class="text-muted small">Rp{{ number_format($hargaAsli, 0, ',', '.') }}</del>
                                        <div class="text-info font-weight-bold h5 mb-0">Rp{{ number_format($hargaFinal, 0, ',', '.') }}</div>
                                    @else
                                        <div class="text-info font-weight-bold h5 mb-0">Rp{{ number_format($hargaAsli, 0, ',', '.') }}</div>
                                    @endif
                                </div>
                                <a href="/booking/{{ $mobil->id }}" class="btn btn-info rounded-pill px-3 shadow-sm btn-sm font-weight-bold">Pesan</a>
                            </div>
                        </div>
                    </div>
                </div>
...
<script>
   ...
   var ctx = document.getElementById("myOccupancyChart").getContext('2d');
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                ...
                pointHitRadius: 10,
                pointBorderWidth: 3,
                data: {!! json_encode($dataSewa) !!}, // Data dari Controller
            }],
        },
        options: {
    ...
    $('#formLaporAdmin').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{ route('laporan.admin.store') }}", // Ganti dengan route POST kamu
            type: "POST",
    ...
    <div class="container text-center pb-4">
        <div class="footer-content">
            <p class="mb-0 copyright-text">
                ...
                <span>Copyright &copy; {{ date('Y') }} <strong>MobiRent</strong>.</span>
