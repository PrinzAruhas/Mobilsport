    ...
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-5 bg-white p-4 shadow-sm" style="border-radius: 25px;">
            <div>
                ...
                <p class="text-muted mb-0 font-weight-bold">Total Anggota: {{ count($users) }}</p>
            </div>
            ...
            <div class="col-xl-3 col-lg-4 col-md-6 mb-5">
                <div class="card user-card h-100 shadow-sm">
                    ...
                    <div class="avatar-wrapper">
                        @if($user->avatar)
                            <img src="{{ asset('storage/' . $user->avatar) }}" class="user-avatar-main shadow">
                        @else
                            <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=a2d2ff&color=fff&bold=true" class="user-avatar-main shadow">
                        @endif
                    </div>

                    <div class="card-body text-center">
                        <h5 class="user-name mb-1 text-truncate">{{ $user->name }}</h5>
                        <p class="user-email text-truncate mb-0">{{ $user->email }}</p>
                        ...
                        <div class="btn-group-custom">
                            <button class="btn-circle-action btn-edit" title="Ubah Akses" data-toggle="modal" data-target="#modalEdit{{ $user->id }}">
                                <i class="fas fa-shield-alt"></i>
                            </button>
                            <button class="btn-circle-action btn-delete" title="Hapus User" onclick="confirmDelete('{{ $user->id }}')">
                ...
                <div class="modal fade" id="modalEdit{{ $user->id }}" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        ...
                        <form action="{{ route('users.update', $user->id) }}" method="POST" class="role-update-form">
    @csrf
    @method('PUT')
    <div class="modal-body p-4 text-center">
        <div class="role-icon-preview mb-3" style="font-size: 3rem; filter: drop-shadow(0 10px 15px rgba(0,0,0,0.1));">
            <i class="fas {{ $roleConfig['icon'] }} text-primary animate__animated animate__bounceIn"></i>
        </div>

        <div class="mb-4">
            <h6 class="font-weight-bold mb-0">{{ $user->name }}</h6>
            <span class="text-muted small">ID Anggota: #{{ $user->id }}</span>
        </div>
        ...
        <div class="form-group text-left">
            ...
            <select name="role_id" class="form-control border-0 bg-light role-select" 
                ...
                <option value="1" {{ $roleId == 1 ? 'selected' : '' }}>🛡️ ADMINISTRATOR</option>
                <option value="2" {{ $roleId == 2 ? 'selected' : '' }}>🔧 STAFF TEKNIS</option>
                <option value="3" {{ $roleId == 3 ? 'selected' : '' }}>👥 PEMINJAM / USER</option>
            </select>
        </div>
    </div>
