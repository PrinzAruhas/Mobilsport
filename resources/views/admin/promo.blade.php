        ...
        <div class="col-xl-7">
            <div class="card glass-card border-0 mb-4 h-100">
                <div class="card-header bg-transparent border-0 px-4 pt-4 d-flex justify-content-between align-items-center">
                    ...
                    <span class="badge badge-primary px-3 py-2" style="border-radius: 10px">{{ count($mobils) }} Unit Aktif</span>
                </div>
                                ...
                                <tr>
                                    <td class="px-4 align-middle">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div>
                                                <span class="d-block font-weight-bold text-dark mb-0" style="font-size: 0.9rem">{{ $m->merek }}</span>
                                                <small class="badge-target">{{ $m->category->nama_kategori }}</small>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 align-middle harga-normal" data-harga="{{ $m->harga_sewa }}" style="font-size: 0.85rem">
                                    ...
                                    <form action="{{ route('promo.update') }}" method="POST">
                                        ...
                                        <td class="align-middle">
                                            <input type="number" name="diskon_mobil" class="form-control premium-input input-diskon text-center w-100" value="{{ $m->diskon }}" min="0" max="100">
                                        </td>
                                        <td class="align-middle">
                                            <input type="date" name="expired_at" class="form-control premium-input expiry-date-input input-expiry w-100" 
                                                   value="{{ $m->expired_at ? \Carbon\Carbon::parse($m->expired_at)->format('Y-m-d') : '' }}"
                                                   min="2026-04-04">
                                        ...
                                        <td class="px-4 align-middle harga-akhir-tampil font-weight-bold text-primary" style="font-size: 0.9rem">
                                            Rp{{ number_format($m->harga_sewa - ($m->harga_sewa * $m->diskon / 100), 0, ',', '.') }}
                                        </td>
        ...
        <div class="col-xl-5">
            <div class="card glass-card border-0 mb-4" style="background: rgba(255,255,255,0.4)">
                <div class="card-body">
                    ...
                    <form action="{{ route('promo.update') }}" method="POST" class="row no-gutters">
                        @csrf
            ...
            <div class="card glass-card border-0 mb-4 shadow-lg" style="background: linear-gradient(135deg, #0f172a 0%, #1e293b 100%); color: white;">
                <div class="card-body p-4">
                    ...
                    <form action="{{ route('promo.store_code') }}" method="POST">
                        @csrf
                        <div class="row">
                ...
                <div class="card-body px-0">
                    <div class="table-responsive custom-scroll" style="max-height: 300px;">
                        <table class="table premium-table m-0">
                            ...
                            <tbody>
                                @php $activePromos = \App\Models\Promo::latest()->get(); @endphp
                                @foreach($activePromos as $promo)
                                <tr>
                                    <td class="px-4 align-middle">
                                        <span class="badge-code">{{ $promo->code }}</span>
                                    </td>
                                    <td class="px-4 align-middle text-danger font-weight-bold">{{ $promo->discount_percent }}%</td>
                                    <td class="px-4 align-middle">
                                        <small class="badge-target text-capitalize">{{ $promo->type }}</small>
                                    </td>
                                    <td class="px-4 align-middle text-center">
                                        <form action="{{ route('promo.delete_code', $promo->id) }}" method="POST" onsubmit="return confirm('Hapus voucher ini?')">
                                            @csrf @method('DELETE')
...
<script>
    ...
    document.getElementById('targetType').addEventListener('change', function() {
        ...
        if (type === 'all') {
            ...
            if (type === 'category') {
                targetSelect.innerHTML += `@foreach($categories as $cat) <option value="{{ $cat->id }}">{{ $cat->nama_kategori }}</option> @endforeach`;
            } else {
                targetSelect.innerHTML += `@foreach($mobils as $m) <option value="{{ $m->id }}">{{ $m->merek }}</option> @endforeach`;
            }
        }
    });
</script>
