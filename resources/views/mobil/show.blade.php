...
<head>
    ...
    <title>{{ $mobil->merek }} - Detail Premium ❄️</title>
...
<body>
    ...
    <div class="container py-5">
        <div class="d-flex justify-content-between align-items-center mb-5">
            <a href="{{ route('mobil.katalog') }}" class="btn btn-white shadow-sm px-4 py-2" style="border-radius: 50px; background: white; font-weight: 700;">
...
<video id="player-video" width="100%" height="100%" controls style="object-fit: cover;">
    <source src="{{ asset('storage/' . $mobil->video) }}" type="video/mp4">
                    ...
                    <div class="row mb-5">
                        <div class="col-4">
                            <div class="spec-item">
                                ...
                                <h6 class="font-weight-bold mb-0 text-dark">{{ $mobil->kapasitas ?? '5' }} Kursi</h6>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="spec-item">
                                ...
                                <h6 class="font-weight-bold mb-0 text-dark">{{ $mobil->transmisi ?? 'Automatic' }}</h6>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="spec-item">
                                ...
                                <h6 class="font-weight-bold mb-0 text-dark">{{ $mobil->bahan_bakarnya ?? 'Bensin' }}</h6>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <div class="description-text p-4 rounded-lg" style="background: rgba(255,255,255,0.4); border-left: 5px solid var(--ice-primary);">
                            {{ $mobil->deskripsi ?? 'Setiap armada telah melalui proses sanitasi kristal sebelum diserahkan kepada Anda. Nikmati kenyamanan berkendara dengan performa mesin tangguh.' }}
                        </div>
                    </div>
            ...
            <div class="col-lg-4">
                <div class="glass-card p-4 sticky-sidebar shadow-lg">
                    <div class="mb-4">
                        <h2 class="font-weight-extrabold text-dark mb-1">{{ $mobil->merek }}</h2>
                        <h5 class="text-info font-weight-bold">{{ $mobil->model }}</h5>
                    </div>
    ...
    <div class="d-flex flex-column align-items-center">
        ...
        {{-- Tampilan Harga Coret (Muncul jika ada diskon unit ATAU diskon redeem) --}}
        @if($mobil->diskon > 0 || $diskonRedeem > 0)
            ...
            <div class="d-flex align-items-baseline">
                <span class="h2 mb-0 font-weight-extrabold text-white">Rp{{ number_format($hargaAkhir, 0, ',', '.') }}</span>
            ...
            <div class="d-flex flex-column align-items-center mt-1" style="gap: 4px;">
                ...
                @if($mobil->diskon > 0)
                    <span class="badge badge-warning px-3 py-1 shadow-sm" style="border-radius: 50px; font-size: 0.7rem;">
                        <i class="fas fa-fire mr-1"></i> UNIT DISKON {{ $mobil->diskon }}%
                    </span>
                @endif
                ...
                @if($diskonRedeem > 0)
                    <span class="badge badge-success px-3 py-1 shadow-sm" style="border-radius: 50px; font-size: 0.7rem;">
                        <i class="fas fa-ticket-alt mr-1"></i> PROMO REDEEM {{ $diskonRedeem }}%
                    </span>
                @endif
            </div>
        @else
            ...
            <div class="d-flex align-items-baseline mt-1">
                <span class="h2 mb-0 font-weight-extrabold text-white">Rp{{ number_format($hargaAwal, 0, ',', '.') }}</span>
                <span class="ml-1 font-weight-bold opacity-75 text-white">/hari</span>
            </div>
        @endif
    </div>
                    ...
                    <form action="{{ route('sewa.create') }}" method="GET" id="bookingForm">
                        ...
                        <div class="form-group mb-4">
                            ...
                            <div class="p-2" style="max-height: 250px; overflow-y: auto; border: 2px solid #e1eefb; border-radius: 12px; background: rgba(255,255,255,0.5);">
                                ...
                                <label class="unit-card d-flex align-items-center p-2 mb-2 rounded shadow-sm m-0" style="background: white; cursor: pointer; border: 1px solid #e1eefb;">
                                    <input type="checkbox" name="unit_ids[]" value="{{ $unit->id }}" class="unit-checkbox mr-3" style="width: 20px; height: 20px; accent-color: var(--ice-primary);">
                                    
                                @if($unit->foto_unit)
<img src="{{ asset('storage/' . $unit->foto_unit) }}"
     class="rounded mr-3"
     style="width: 70px; height: 50px; object-fit: cover;"
     onclick="showZoom('{{ asset('storage/' . $unit->foto_unit) }}')">
@else
...
@endif
                                    ...
                                    <div class="flex-grow-1">
                                        <span class="d-block font-weight-bold text-dark" style="font-size: 0.95rem;">{{ $unit->no_polisi }}</span>
                                        <span class="badge badge-info" style="background: var(--ice-gradient);">{{ $unit->warna }}</span>
                                    </div>
                                </label>
                                @empty
    ...
    <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <span class="text-dark small font-weight-bold">Rp{{ number_format($mobil->harga_sewa, 0, ',', '.') }}</span>
    </div>

    @if($mobil->diskon > 0)
    <div class="d-flex justify-content-between align-items-center mb-2">
        <span class="text-muted small font-weight-bold">Diskon Unit</span>
        <span class="text-danger small font-weight-bold">-{{ $mobil->diskon }}%</span>
    </div>
    @endif

    @if(isset($diskonRedeem) && $diskonRedeem > 0)
    <div class="d-flex justify-content-between align-items-center mb-2">
        <span class="text-muted small font-weight-bold">Kode Promo ({{ session('applied_promo')['code'] ?? 'Aktif' }})</span>
        <span class="text-success small font-weight-bold">-{{ $diskonRedeem }}%</span>
    </div>
    @endif
   ...
   <div class="comment-section">
    ...
    <div class="comments-list">
        @forelse($mobil->comments()->whereNull('parent_id')->orderBy('created_at', 'desc')->get() as $comment)
                            ...
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    <h6 class="font-weight-bold mb-0 text-dark">
                                        {{ $comment->user->name }}
                                        @if($comment->user->role_id == 1) <span class="badge-role">Admin</span> @endif
                                    </h6>
<span class="comment-date">
    ...
    @endif
                            ...
                            <p class="comment-text mt-3 mb-0">{{ $comment->isi_komentar }}</p>
                            
                            @if(Auth::check())
                            <a href="javascript:void(0)" class="btn-reply-link" onclick="showReplyForm({{ $comment->id }})">
                    ...
                    <div id="reply-form-{{ $comment->id }}" class="mt-4 d-none">
                        <form action="{{ route('komentar.store') }}" method="POST" class="p-3 rounded-lg" style="background: #f8fbff; border: 1px solid #e1eefb;">
                            @csrf
                            <input type="hidden" name="mobil_id" value="{{ $mobil->id }}">
                            ...
                            <div class="text-right">
                                <button type="button" class="btn btn-sm text-muted mr-2" onclick="showReplyForm({{ $comment->id }})">Batal</button>
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between">
                                        <span class="font-weight-bold small text-dark">
                                            {{ $reply->user->name }}
                                            @if($reply->user->role_id == 1) <span class="badge-role">Admin</span> @endif
                                        </span>
                                        <span class="comment-date" style="font-size: 0.65rem;">{{ $reply->created_at->diffForHumans() }}</span>
                                    </div>
                                    <p class="comment-text mb-0 mt-1 small text-secondary">{{ $reply->isi_komentar }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
...
$(document).ready(function() {
    ...
    const hargaAsli = {{ $mobil->harga_sewa }};
    const persenDiskonUnit = {{ $mobil->diskon ?? 0 }};
    // Mengambil diskon redeem dari session/variabel PHP
    const persenDiskonRedeem = {{ $diskonRedeem ?? 0 }};
